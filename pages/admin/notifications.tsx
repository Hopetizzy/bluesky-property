...
type AdminFilterTab = 'all' | 'unread' | 'applications' | 'payments' | 'properties' | 'system';
...
export default function AdminNotificationsPage() {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // 1. Local store items (Admin-only)
      const localNotifs = store.getNotifications().filter(
        (n) => n.role === 'admin' || (!n.role && !n.profile_id && (n.type === 'application' || n.type === 'payment' || n.type === 'property' || n.type === 'system'))
      );

      // 2. Supabase records (Admin-only)
      if (isSupabaseConfigured()) {
        ...
        if (dbNotifs && dbNotifs.length > 0) {
          const mappedDb: NotificationItem[] = dbNotifs
            .filter((n: any) => n.role === 'admin' || (!n.role && !n.profile_id && (n.type === 'application' || n.type === 'payment' || n.type === 'property' || n.type === 'system')))
            .map((n: any) => ({
              id: String(n.id),
              user_id: n.profile_id || n.user_id || 'admin-1',
      ...
      console.warn('Admin notifications load note:', err);
  ...
  const handleMarkAllRead = async () => {
    ...
    store.markAllNotificationsAsRead('admin');
  ...
  const filteredNotifications = useMemo(() => {
    return notifications.filter((item) => {
      ...
      if (activeTab === 'payments') return item.type === 'payment' || item.title.toLowerCase().includes('payment') || item.title.toLowerCase().includes('fee') || item.title.toLowerCase().includes('plan');
      if (activeTab === 'properties') return item.type === 'property' || item.title.toLowerCase().includes('property') || item.title.toLowerCase().includes('listing');
  ...
  const getNotificationIcon = (type: string, title: string) => {
    ...
    if (t === 'payment' || tit.includes('payment') || tit.includes('fee') || tit.includes('proof')) {
    ...
    if (t === 'success' || tit.includes('verified') || tit.includes('approved')) {
      return <CheckCircle2 size={20} color="var(--color-success)" />;
    }
    if (t === 'warning' || tit.includes('alert') || tit.includes('reject')) {
      return <AlertTriangle size={20} color="var(--color-warning)" />;
    }
    return <Shield size={20} color="var(--color-primary-navy)" />;
  };
  ...
  return (
    <AppLayout title="Admin Notification Center | Blue Sky Property" headerTitle="Admin Notifications">
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--color-primary-navy)', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                <ShieldAlert size={22} color="var(--color-primary-navy)" />
                Admin Notification Center
              </h1>
              <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', margin: '2px 0 0' }}>
                System-wide audit alerts, pending payment proofs, application reviews, and moderation events
              </p>
            </div>
          </div>
        ...
        >
          {[
            ...
            { id: 'payments', label: 'Payment Proofs', count: notifications.filter((n) => n.type === 'payment' || n.title.toLowerCase().includes('payment') || n.title.toLowerCase().includes('fee')).length },
            { id: 'properties', label: 'Property Moderation', count: notifications.filter((n) => n.type === 'property' || n.title.toLowerCase().includes('property')).length },
            { id: 'system', label: 'System & Security', count: notifications.filter((n) => n.type === 'system' || n.type === 'info').length },
          ].map((tab) => {
            const isActive = activeTab === tab.id;
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as AdminFilterTab)}
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  gap: 6,
        ...
        {isLoading ? (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>Loading admin notifications...</p>
          </div>
          ...
          >
            ...
            <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', margin: '0 0 20px', maxWidth: 400, marginLeft: 'auto', marginRight: 'auto' }}>
              All administrative operations are up to date. You will be alerted here whenever new payments are uploaded, applications are filed, or properties are submitted for verification.
            </p>
            {activeTab !== 'all' && (
              <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {filteredNotifications.map((notif) => {
              ...
              const targetUrl = notif.link_url || (notif.type === 'application' ? '/admin/applications' : notif.type === 'payment' ? '/admin/payments' : '/admin');
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--color-border)', paddingTop: 8 }}>
                      ...
                      >
                        Action in Portal <ExternalLink size={12} />
                      </span>
