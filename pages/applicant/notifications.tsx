...
type ApplicantFilterTab = 'all' | 'unread' | 'applications' | 'payments' | 'system';

export default function ApplicantNotificationsPage() {
  const router = useRouter();
  const [notifications, setNotifications] = useState<NotificationItem[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [activeTab, setActiveTab] = useState<ApplicantFilterTab>('all');
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      // 1. Local store items (Applicant-only)
      const localNotifs = store.getNotifications().filter(
        (n) => n.role === 'applicant'
      );

      // 2. Supabase records (Applicant-only)
      if (isSupabaseConfigured()) {
        ...
        if (dbNotifs && dbNotifs.length > 0) {
          const mappedDb: NotificationItem[] = dbNotifs
            .filter((n: any) => n.role === 'applicant')
            .map((n: any) => ({
              id: String(n.id),
              user_id: n.profile_id || n.user_id || 'applicant-1',
      ...
      console.warn('Applicant notifications load note:', err);
  ...
  const handleMarkAllRead = async () => {
    ...
    store.markAllNotificationsAsRead('applicant');
  ...
  const filteredNotifications = useMemo(() => {
    return notifications.filter((item) => {
      ...
      if (activeTab === 'payments') return item.type === 'payment' || item.title.toLowerCase().includes('payment') || item.title.toLowerCase().includes('fee');
  ...
  const getNotificationIcon = (type: string, title: string) => {
    ...
    if (t === 'payment' || tit.includes('payment') || tit.includes('fee')) {
  ...
  return (
    <AppLayout title="My Notifications | Blue Sky Property" headerTitle="My Notifications">
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px 80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', margin: '2px 0 0' }}>
                Status updates for your rental applications, identity verification, and payment receipts
              </p>
            </div>
          </div>
        ...
        >
          {[
            ...
            { id: 'payments', label: 'Payments & Fees', count: notifications.filter((n) => n.type === 'payment' || n.title.toLowerCase().includes('payment') || n.title.toLowerCase().includes('fee')).length },
            { id: 'system', label: 'System Alerts', count: notifications.filter((n) => n.type === 'system' || n.type === 'info').length },
          ].map((tab) => {
            const isActive = activeTab === tab.id;
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as ApplicantFilterTab)}
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  gap: 6,
        ...
        {isLoading ? (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>Loading notifications...</p>
          </div>
          ...
          >
            ...
            <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', margin: '0 0 20px', maxWidth: 360, marginLeft: 'auto', marginRight: 'auto' }}>
              You will receive real-time notifications here when your rental applications are reviewed, payments are confirmed, or viewing tours are booked.
            </p>
            {activeTab !== 'all' && (
              <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {filteredNotifications.map((notif) => {
              ...
              const targetUrl = notif.link_url || '/applicant/applications';
